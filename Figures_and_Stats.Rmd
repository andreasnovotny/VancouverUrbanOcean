...
```{r}
All <- model_NMDS(Months, Locations, "Relative Abundance")

Alpha_diversity <-
  tibble(Sample_ID = rownames(All$Matrix),
         Richness = specnumber(All$Matrix),
         Shannon_Index = diversity(All$Matrix, index = "shannon"),
         Simpson_Index = diversity(All$Matrix, index = "simpson"),
         Evenness = diversity(All$Matrix, index = "shannon") / log(specnumber(All$Matrix))
       
       ) %>% 
  left_join(select(All$Data, 1:5)) %>% 
  mutate(Month = factor(Month, levels = Months))
